...
1. **Tag query semantics** — ~~AND vs OR: the current design requires all requested tags to be present (AND). Should `artifact_search` support OR semantics with an explicit `--any-tag` flag? AND is stricter and more useful for filtering `production` + `team-a`; OR is better for broad discovery.~~ **Resolved:** default query semantics is AND (all requested tags must be present). A future `--any-tag` flag will enable OR semantics for broad discovery. AND is the correct default for filtering specific intersections like `production` + `team-a`.
...
3. **Artifact FTS** — ~~should artifact names and metadata be indexed in an FTS5 table for full-text search, similar to `memories_fts`? The current design uses exact tag matching and SQL `LIKE` on `name`. A separate `artifacts_fts` virtual table would support freetext queries but adds maintenance complexity.~~ **Resolved:** deferred. SQL LIKE on artifact name is sufficient for MVP. An `artifacts_fts` virtual table will be added only if query performance on name/metadata becomes a bottleneck.
...
5. **Importance decay schedule** — ~~what is the decay period before `high → moderate` and `moderate → low`? Candidate: 30 days without an `access_log` entry for `high`, 14 days for `moderate`. These values affect recall quality and need empirical validation.~~ **Resolved:** decay periods are 30 days without `access_log` entry for high→moderate, 60 days for moderate→low. These values are configurable in `config.toml` under `[memory.decay]`. The sweep runs as a scheduled task.

6. **Multi-agent namespace boundary enforcement** — ~~the current `workspace_id` approach is permissive: any agent that knows a `workspace_id` can query its memories. How should access be enforced? Options: FK to an `agent_workspace_access` table, or a per-memory ACL column.~~ **Resolved:** enforce access via an `agent_workspace_access` table (`agent_id TEXT, workspace_id INTEGER, granted_at TEXT, PRIMARY KEY(agent_id, workspace_id)`). Agents can only query memories in workspaces they have explicit grants for. The `workspace_id` approach remains permissive only for backward compatibility during migration.

**P7 — Agent Lifecycle & Versioning**

7. **Upgrade enforcement vs. advisory** — ~~the current design is advisory only. Should there be a mechanism to refuse `agent_heartbeat` for agents running skills older than N days, forcing a restart? This prevents indefinitely stale agents but may cause unexpected outages.~~ **Resolved:** advisory-only. The server will NOT refuse heartbeats for agents running stale skills. Forcing restarts risks unexpected outages. The `upgrade_available` flag and room notification are sufficient — operators can enforce upgrade policies externally if needed.

8. **Template versioning** — ~~`agent_templates` has no version history. If a template changes after agents are already instantiated from it, those agents silently diverge from the new template. Should templates be immutable (new template required for each change), or should `agent_instantiate` record the template state at spawn time?~~ **Resolved:** templates are immutable. Changing a template requires creating a new record (new UUIDv7 ID). `agent_instantiate` records the `template_id` at spawn time, so agents never silently diverge from an updated template. Old templates can be archived but not modified.

9. **Skill path portability** — ~~`skills_json` stores absolute filesystem paths. If Nous is moved to a different machine or the skills directory is renamed, the stored paths become invalid and rollback/upgrade detection breaks. Should paths be stored relative to a configurable `skills_root`?~~ **Resolved:** skill paths in `skills_json` are stored relative to a configurable `skills_root` directory. Default `skills_root` is the Paseo plugin directory. This ensures portability across machines and directory renames.
