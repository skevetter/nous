...
fn context_returns_workspace_scoped() {
    ...
    let entries = db.context(ws_a_id, false, 50, 0, None, None).unwrap();
...
fn context_summary_omits_content() {
    ...
    let entries = db.context(ws_id, true, 50, 0, None, None).unwrap();
    assert_eq!(entries.len(), 1);
    assert!(entries[0].content.is_none());

    let entries_full = db.context(ws_id, false, 50, 0, None, None).unwrap();
...
fn context_excludes_archived() {
    ...
    let entries = db.context(ws_id, false, 50, 0, None, None).unwrap();
    assert_eq!(entries.len(), 1);
