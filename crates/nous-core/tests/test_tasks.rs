...
async fn test_create_task_minimal() {
    ...
    let task = tasks::create_task(&pools.fts, "Minimal task", None, None, None, None, None, false, None).await.unwrap();
...
async fn test_create_task_empty_title_fails() {
    ...
    let result = tasks::create_task(&pools.fts, "  ", None, None, None, None, None, false, None).await;
...
async fn test_create_task_with_room_creation() {
    ...
    let task = tasks::create_task(&pools.fts, "Task with room", None, None, None, None, None, true, None).await.unwrap();

    assert!(task.room_id.is_some(), "room_id should be set when create_room=true");
...
async fn test_list_tasks_filter_by_status() {
    ...
    let t1 = tasks::create_task(&pools.fts, "Open task", None, None, None, None, None, false, None).await.unwrap();
    tasks::create_task(&pools.fts, "Another open", None, None, None, None, None, false, None).await.unwrap();
    tasks::close_task(&pools.fts, &t1.id, None).await.unwrap();

    let open_tasks = tasks::list_tasks(&pools.fts, Some("open"), None, None, None, None, None, None).await.unwrap();
    assert_eq!(open_tasks.len(), 1);
    assert_eq!(open_tasks[0].title, "Another open");

    let closed_tasks = tasks::list_tasks(&pools.fts, Some("closed"), None, None, None, None, None, None).await.unwrap();
...
async fn test_list_tasks_filter_by_assignee() {
    ...
    tasks::create_task(&pools.fts, "Alice task", None, None, Some("alice"), None, None, false, None).await.unwrap();
    tasks::create_task(&pools.fts, "Bob task", None, None, Some("bob"), None, None, false, None).await.unwrap();

    let alice_tasks = tasks::list_tasks(&pools.fts, None, Some("alice"), None, None, None, None, None).await.unwrap();
...
async fn test_list_tasks_filter_by_label() {
    ...
    tasks::create_task(&pools.fts, "Bug task", None, None, None, Some(&labels_bug), None, false, None).await.unwrap();
    tasks::create_task(&pools.fts, "Feature task", None, None, None, Some(&labels_feat), None, false, None).await.unwrap();

    let bug_tasks = tasks::list_tasks(&pools.fts, None, None, Some("bug"), None, None, None, None).await.unwrap();
...
async fn test_list_tasks_pagination() {
    ...
    for i in 0..5 {
        tasks::create_task(&pools.fts, &format!("Task {i}"), None, None, None, None, None, false, None).await.unwrap();
    }

    let page1 = tasks::list_tasks(&pools.fts, None, None, None, Some(2), Some(0), None, None).await.unwrap();
    assert_eq!(page1.len(), 2);

    let page2 = tasks::list_tasks(&pools.fts, None, None, None, Some(2), Some(2), None, None).await.unwrap();
    assert_eq!(page2.len(), 2);

    let page3 = tasks::list_tasks(&pools.fts, None, None, None, Some(2), Some(4), None, None).await.unwrap();
...
async fn test_update_task_status() {
    ...
    let task = tasks::create_task(&pools.fts, "Status test", None, None, None, None, None, false, None).await.unwrap();
    let updated = tasks::update_task(&pools.fts, &task.id, Some("in_progress"), None, None, None, None, Some("actor-1")).await.unwrap();
    assert_eq!(updated.status, "in_progress");

    let events = tasks::task_history(&pools.fts, &task.id, None).await.unwrap();
    assert!(events.iter().any(|e| e.event_type == "status_changed" && e.new_value.as_deref() == Some("in_progress")));
...
async fn test_update_task_priority() {
    ...
    let task = tasks::create_task(&pools.fts, "Priority test", None, None, None, None, None, false, None).await.unwrap();
    let updated = tasks::update_task(&pools.fts, &task.id, None, Some("critical"), None, None, None, None).await.unwrap();
    assert_eq!(updated.priority, "critical");

    let events = tasks::task_history(&pools.fts, &task.id, None).await.unwrap();
    assert!(events.iter().any(|e| e.event_type == "priority_changed" && e.new_value.as_deref() == Some("critical")));
...
async fn test_update_task_assignee() {
    ...
    let task = tasks::create_task(&pools.fts, "Assignee test", None, None, None, None, None, false, None).await.unwrap();
    let updated = tasks::update_task(&pools.fts, &task.id, None, None, Some("new-agent"), None, None, None).await.unwrap();
    assert_eq!(updated.assignee_id.as_deref(), Some("new-agent"));

    let events = tasks::task_history(&pools.fts, &task.id, None).await.unwrap();
    assert!(events.iter().any(|e| e.event_type == "assigned" && e.new_value.as_deref() == Some("new-agent")));
...
async fn test_close_task() {
    ...
    let task = tasks::create_task(&pools.fts, "Close me", None, None, None, None, None, false, None).await.unwrap();
    let closed = tasks::close_task(&pools.fts, &task.id, Some("closer")).await.unwrap();
...
async fn test_link_then_unlink() {
    ...
    let t1 = tasks::create_task(&pools.fts, "Task 1", None, None, None, None, None, false, None).await.unwrap();
    let t2 = tasks::create_task(&pools.fts, "Task 2", None, None, None, None, None, false, None).await.unwrap();

    let link = tasks::link_tasks(&pools.fts, &t1.id, &t2.id, "blocked_by", None).await.unwrap();
    ...
    tasks::unlink_tasks(&pools.fts, &t1.id, &t2.id, "blocked_by", None).await.unwrap();
...
async fn test_add_note_requires_room() {
    ...
    let task = tasks::create_task(&pools.fts, "No room task", None, None, None, None, None, false, None).await.unwrap();
...
async fn test_add_note_with_room() {
    ...
    let task = tasks::create_task(&pools.fts, "Room task", None, None, None, None, None, true, None).await.unwrap();
    let note = tasks::add_note(&pools.fts, &task.id, "agent-1", "This is a note").await.unwrap();
...
async fn test_task_history() {
    ...
    let task = tasks::create_task(&pools.fts, "History task", None, None, None, None, None, false, Some("actor")).await.unwrap();
    tasks::update_task(&pools.fts, &task.id, Some("in_progress"), None, None, None, None, Some("actor")).await.unwrap();
    tasks::close_task(&pools.fts, &task.id, Some("actor")).await.unwrap();
...
async fn test_search_tasks_finds_by_title() {
    ...
    tasks::create_task(&pools.fts, "Optimize database queries", None, None, None, None, None, false, None).await.unwrap();
    tasks::create_task(&pools.fts, "Fix login page", None, None, None, None, None, false, None).await.unwrap();

    let results = tasks::search_tasks(&pools.fts, "database", None).await.unwrap();
