...
use nous_core::types::{Category, CategorySource, Memory, NewMemory, Relationship};
    ...
    match format {
        ...
        OutputFormat::Csv => {
            print_csv(
                &["metric", "value"],
                &[
                    vec!["db_path".into(), config.memory.db_path.clone()],
                    vec!["memories".into(), memory_count.to_string()],
                    vec!["archived".into(), archived_count.to_string()],
                    vec!["categories".into(), category_count.to_string()],
                    vec!["chunks".into(), chunk_count.to_string()],
                    vec!["active_model".into(), model_name.to_string()],
                ],
            )?;
        }
        OutputFormat::Human => {
