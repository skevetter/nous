...
pub fn run_trace(
    ...
    let otlp_db_path = expand_tilde(&config.otlp.db_path);
    if otlp_db_path.is_empty() {
        return Err("OTLP database not configured".into());
    }
    if !Path::new(&otlp_db_path).exists() {
        return Err(format!("OTLP database not found at {otlp_db_path}").into());
    }

    let db_key = config.resolve_db_key().ok();
    let mem_db = MemoryDb::open(&config.memory.db_path, db_key.as_deref(), 384)?;
    let otlp_db = nous_otlp::db::OtlpDb::open(&otlp_db_path, None)?;
    ...
    if let Some(tid) = trace_id {
        ...
        let trace = recalled.memory.trace_id.as_deref();
        let session = recalled.memory.session_id.as_deref();

        if trace.is_none() && session.is_none() {
            return Err("memory has no trace_id or session_id for OTLP correlation".into());
        }

        let spans = match trace {
            Some(tid) => otlp_db.query_spans(tid, None, None)?,
            None => vec![],
        };
        ...
        let output = serde_json::json!({
            "memory": {
                ...
                "title": recalled.memory.title,
                "content": recalled.memory.content,
                "memory_type": recalled.memory.memory_type,
                "session_id": recalled.memory.session_id,
                "trace_id": recalled.memory.trace_id,
                "created_at": recalled.memory.created_at,
                "tags": recalled.tags,
